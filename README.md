...
### Measure batch size on DGX

After the ongoing run finishes, use the same environment and the run's actual
`resolved_config.json` (YAML is also accepted). Run from the repository root on
an otherwise idle GPU; this command does not stop another training process:

```bash
python -m scripts.profile_batches --config outputs/YOUR_RUN/resolved_config.json --output outputs/batch_profile_01
```

The profiler runs batch sizes 4, 8, 12, and 16 in separate child processes. Each
candidate runs every configured task with 3 warm-up and 10 measured optimizer
steps per task. It preserves image size (512 in the DGX configs), model, FP32
training, learning rates, loss weights, workers, task masks and replay settings.
Replay uses real earlier-task tiles and teacher logits generated by the bounded
training trajectory, not a fully trained checkpoint. Later-task auxiliary losses
and replay/distillation are included. This is a workload test, not a quality test.

`report.json` records synchronized step time (including batch loading and metrics),
new-task images/second, peak allocated/reserved CUDA bytes, GPU capacity and
failures. Peaks include warm-up and task setup. Each child's full output is in
`batch_N/worker.log`. OOM or another failure stops the sweep; a timeout defaults
to 1800 seconds per child and can be changed with `--timeout`. The report retains
completed candidates even when a larger batch fails.

The smallest batch within 5% of the best throughput among candidates with peak
reserved memory strictly below 85% of device capacity is selected. A separate
child then runs 3 warm-up and 3 measured training steps per task plus validation
on up to two batches of held-out tiles, including binary threshold calibration.
Finite training loss, finite global gradient norm, finite validation loss, and
the memory limit must pass before `recommended.yaml` is written. The original
configuration and run outputs are never modified. The recommendation retains
the learning rates and uses a fresh `recommended_training` directory inside the
profile output. Run it with:

```bash
python -m tqsi.train --config outputs/batch_profile_01/recommended.yaml
```

Use a new profiler output directory each time. Compare validation quality in the
new training run: increasing batch size reduces optimizer updates per epoch.
The profiler does not certify a Dice target or automatically launch full training.
